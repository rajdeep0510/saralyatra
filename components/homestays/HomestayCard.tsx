...
import ArtisanExperienceModal from "./ArtisanExperienceModal";
...
export default function HomestayCard({ homestay, currentLang }: HomestayCardProps) {
    ...
    <>
      <div className="rounded-2xl bg-white border border-stone-200 p-6 hover:border-stone-300 hover:shadow-[0_12px_28px_rgba(28,25,23,0.06)] transition-all duration-300 flex flex-col justify-between gap-5 relative">
        ...
        <div>
          ...
          {/* Artisan Workshop Mini Preview Ribbon */}
          <div className="mt-3 p-2.5 rounded-xl bg-amber-500/10 border border-amber-300 text-xs flex items-center justify-between">
            <div className="flex items-center gap-1.5 text-[11px] text-amber-900 font-medium">
              <span>🎨</span>
              <span>On-site Artisan Craft Workshop Available</span>
            </div>
            <button
              type="button"
        ...
        {/* Action footer */}
        <div className="flex items-center justify-between gap-3 pt-4 border-t border-stone-100 flex-wrap">
          <div>
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => setIsArtisanModalOpen(true)}
              className="px-3.5 py-2.5 rounded-xl border border-amber-400 bg-amber-50 text-amber-900 hover:bg-amber-100 font-bold text-xs transition-all cursor-pointer shadow-xs"
              title="Book Artisan Workshop & Direct Host Inquiry"
            >
      ...
      {/* Artisan Workshop & Host Inquiry Modal */}
      <ArtisanExperienceModal
